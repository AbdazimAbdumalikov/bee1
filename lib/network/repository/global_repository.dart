...
class GlobalRepository {
  ...
  Future<List<OrderDTO>> getOrdersByCities() async =>
      _networkService.getOrdersByCities();
  ...
  Future<MakeEmployeePartnerDTO?> getEmployeeByRef({required String ref}) async {
    try {
      final List<MakeEmployeePartnerDTO> employees = await _networkService.getEmployeeByRef(ref: ref);
      // Проверяем, есть ли элементы в списке
      if (employees.isNotEmpty) {
        // Возвращаем первый элемент из списка
      ...
      print("Error getting employee by ref: $e");
      rethrow;
    }
  }
  ...
  Future<MakeEmployeePartnerDTO> makeEmployeePartner({
    ...
    try {
      final MakeEmployeePartnerDTO response = await _networkService.makeEmployeePartner(
        employeeId: employeeId,
      );
      return response;
    } catch (e) {
      // Обработка ошибок
      print("Error making employee partner: $e");
      rethrow;
    }
  }
}
