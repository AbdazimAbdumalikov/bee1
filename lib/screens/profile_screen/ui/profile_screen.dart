...
import 'package:europharm_flutter/screens/search_by_ref_screen/search_by_ref_screen.dart';
...
import '../../my_partners/my_partners_screen.dart';
        ...
        body: Padding(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              //             children: [
              //               Text(user?.name),
              //               // Далее ваш код для отображения остальных данных профиля
              //             ],
              //           ),
              //         );
              //       } else {
...
class _BuildUserInfo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<ProfileBloc, ProfileState>(
      ...
      builder: (context, state) {
        if (state is ProfileStateLoaded) {
          print('Profile state loaded: ${state.profile}');
          if (state.profile.partners != null && state.profile.partners!.isNotEmpty) {
            print('First partner: ${state.profile.partners![0]}');
          } else {
                        ...
                        children: [
                          Text(
                            state.profile.id != null &&
                                state.profile.id != null
                                ? "${state.profile.name} "
                                "${state.profile.partners!.first.name}" // Отображаем имя первого партнера
                          ...
                          Text(
                              "${state.profile.logisticCompany?.name}"
                          ),
                          SizedBox(
                            width: 5,
                          ),
                          // ListTile(
                          //   title: Text("Компания: Л компания"),
                          // )
                        ],
                      ),
                    ),
